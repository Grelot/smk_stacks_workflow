...
rule fastqgz_gunzip:
	input:
		R1=config["fastqFolder"]+"/{run}_R1.fastq.gz",
		R2=config["fastqFolder"]+"/{run}_R2.fastq.gz",
	output:
...
rule phix_filter:
	input:
		R1="03-illumina_filter/{run}.i_R1.fastq",
		R2="03-illumina_filter/{run}.i_R2.fastq"
	output:
		R1="04-phix_filter/{run}.i.p_R1.fastq",
		R2="04-phix_filter/{run}.i.p_R2.fastq"
	singularity:
		config["container"]
	log:
		"10-logs/phix_filter/{run}.log"
	shell:
		'''bbduk.sh in1={input.R1} in2={input.R2} out1={output.R1} out2={output.R2} k=31 ref=artifacts,phix ordered cardinality 2> {log}'''
